"""
AMI Connection Manager with Connection Pooling

This module provides a multi-tenant AMI connection manager using Panoramisk 1.4
for fast event detection and call management. It implements connection pooling,
automatic reconnection, failover, and tenant isolation.
"""
...
@dataclass
class AMIConnectionConfig:
    """Configuration for AMI connections per tenant."""
    host: str
    port: int = 5038
    username: str = ""
    secret: str = ""
...
class AMIConnection:
    """Represents a single AMI connection with health monitoring."""
    
    def __init__(self, tenant_id: str, config: AMIConnectionConfig):
    ...
    async def connect(self) -> bool:
        """Establish AMI connection with automatic retry."""
    ...
    async def send_command(self, action: str, **kwargs) -> Optional[Dict[str, Any]]:
    ...
    async def _handle_any_event(self, manager, message):
...
class AMIManager:
    ...
    def __init__(self):
        self.connections: Dict[str, AMIConnection] = {}
        self.session_manager: SessionManager = get_session_manager()
        self._lock = asyncio.Lock()
        
    async def get_connection(self, tenant_id: str) -> Optional[AMIConnection]:
    ...
    async def send_command(self, tenant_id: str, action: str, **kwargs) -> Optional[Dict[str, Any]]:
    ...
    async def register_event_handler(self, tenant_id: str, event_type: str, handler: Callable):
    ...
    async def _get_tenant_ami_config(self, tenant_id: str) -> Optional[AMIConnectionConfig]:
